...
public class FindLocation extends CommandBase {
  ...
  public void updateFromReflectors(){
    ...
    ta = NetworkTableInstance.getDefault().getTable("limelight").getEntry("ta"); //percentage of pixels taken up by the target.
    ...
    double areaAngle = 90 - ((0.5 * ta.getDouble(0)) * 59.6); //the angle that the ta line (always parallel to the face of the camera) makes with the line that connects the camera and the ta plane.
    // As a right triangle, the ta line would be a side that forms a right angle with the line that goes from the camera to where its looking. Its length is determined by ta. The last side is the one mentioned above.
    // The angle that this side makes with the ta side is areaAngle. This angle is useful later because it is supplementary with an angle we need for another triangle.

    double b = (Math.sin((0.5 * ta.getDouble(0) * 59.6)) *
    Variables.getInstance().DistanceFromPowerPortMeters(m_limeLightServo.getAngle()))/Math.sin(areaAngle); //the actual length of ta at the center of the target. The ta line.

    double c = 0.498475; //half of the length of the target. One of the sides on the other triangle.

    double bAngle = Math.asin((b*(Math.sin(180-areaAngle)))/c); // On the new triangle, one side is half of the power port target, one side is b, or the ta line, and the last connects these. Call this a.
